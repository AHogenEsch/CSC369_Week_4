...
def visualize_confined_bots():
    print(f"--- Visualizing confined Users from: {DATA_FILE_PATH} ---")
    start_t = time.perf_counter()

    # 1. Load Data
    lf = pl.scan_parquet(DATA_FILE_PATH)
    
    # Filter out users with very low activity (noise)
    # We need a window context, so we filter basic count first
    lf = lf.filter(pl.len().over("user_id_int") > 10)

    # 2. Identify confined Users

    low_coordinate_deviation = (
        lf.group_by("user_id_int")
        .agg([
            (pl.col("x").max() - pl.col("x").min()).alias("x_range"),
            (pl.col("y").max() - pl.col("y").min()).alias("y_range")
        ])
        .filter(
            (pl.col("x_range") <= 3) & (pl.col("y_range") <= 3)
        )
        .select("user_id_int") # Just keep the IDs for the downstream join/visualization
    )
    
    # 3. Filter Main Data for ONLY these Users
    print("  > Filtering dataset for identified confined bots...")
    
    # Join back to get all pixels placed by these users
    target_pixels = (
        lf.join(low_coordinate_deviation, on="user_id_int", how="inner")
    ...
    user_count = low_coordinate_deviation.collect().height
    pixel_count = final_state.height
    print(f"  > Found {user_count} confined users responsible for {pixel_count} unique pixels.")
    ...
    plotter.add_scalar_bar(title="Limited Spatial Bot Overlaps", mapper=None)
    plotter.show_grid()
    plotter.add_text(f"Spatially confined Bots ({user_count} Users)", position='upper_left', font_size=12)
